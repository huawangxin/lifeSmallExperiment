...
public class GuessingGame {
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (result[0] == chs.length) {
				...
				count++;
				System.out.println("GuessingGame>你猜对" + result[1]
						+ "个字符，    其中" + result[0] + "个字符的位置正确！（总次数=" + count
						+ "，exit——    退出）");
			}
		}
		scanner.close();
	...
	/**
	 * * 随机生成需要猜测的字母序列 * * @return 存储随机字符的数组
	 * */
	...
	/**
	 * * 比较玩家输入的字母序列和程序所生成的字母序列，逐一比较字符及其位置，并记载比较结果 * * @param chs * 程序生成的字符序列 * @param
	 * input * 玩家输入的字符序列 * @return 存储比较的结果。返回值int数组 的长度为2，其中，索引为0的位置 *
